...
const CityBusItem = ({ arrivalBus }: { arrivalBus: BusArrivalProps }) => {
  ...
  if (!arrivalBus.busNumber) {
    return <FontText style={[styles.emptyMessage, { color: colors.light.gray200 }]}>
      30분 안으로 오는 버스가 없어요
    </FontText>
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <FontText fontWeight="700"
                style={[styles.busNumber, { color: colors.dark.contrast }]}>{arrivalBus.busNumber}</FontText>
      <FontText style={[styles.remainingTime, { color: colors.dark.red }]}>
